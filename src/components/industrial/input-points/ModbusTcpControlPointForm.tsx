...
const ModbusTcpControlPointForm: React.FC<ModbusTcpControlPointFormProps> = ({
  points,
  onPointsChange
}) => {
  ...
  const openConfigDialog = (point: ModbusControlPoint) => {
    ...
    setConfigForm({
      ...
      boundInputProtocol: point.boundInputProtocol || 'MODBUS TCP 服务端',
      boundInputPoint: point.boundInputPoint || 'motor_start'
    });
  ...
  const exportConfig = () => {
    ...
    link.href = url;
    link.download = 'modbus_tcp_control_config.json';
    link.click();
    URL.revokeObjectURL(url);
    toast.success('MODBUS TCP控制点位配置已导出');
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        if (config.points) {
          ...
          toast.success('MODBUS TCP控制点位配置已导入');
        } else {
  ...
  const pointOptions = [
    { value: 'motor_start', label: 'motor_start' },
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <CardTitle>MODBUS TCP 控制点位配置</CardTitle>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setConfigForm({ 
                  ...
                  boundInputProtocol: value === 'BIND_INPUT' ? (configForm.boundInputProtocol || 'MODBUS TCP 服务端') : undefined,
                  boundInputPoint: value === 'BIND_INPUT' ? (configForm.boundInputPoint || 'motor_start') : undefined
                })}
              >
                <SelectTrigger>
                  <SelectValue />
...
export default ModbusTcpControlPointForm;